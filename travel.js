(async function () {
 ...
 try {
  ...
  const centers={'United Kingdom':[54,-2],'United States':[39,-98],'Iceland':[65,-19],'Spain':[40,-4],'Portugal':[39.5,-8],'France':[47,2],'Morocco':[32,-7],'Türkiye':[39,35],'United Arab Emirates':[24,54],'Japan':[36,138],'China':[35,105],'Denmark':[56,10],'Hungary':[47,19],'Serbia':[44,21]};
  const countries=[...new Set(places.map(p=>p.group))].map(group=>{const members=places.filter(p=>p.group===group);return {name:group,group,kind:'Country',lat:centers[group][0],lng:centers[group][1],status:members.some(p=>p.status==='visited')?'visited':'next'}});
  const entries=[...places,...countries];
  let currentView='all',overviewZoom=2;
  const markers=entries.map(p=>{
   ...
   if(p.kind==='Country')m.on('click',()=>showCountry(p.group));else m.bindPopup(box);return m;
  });
  ...
  const labels=entries.map((p,i)=>{const b=document.createElement('button');b.type='button';b.className='map-place-label'+(p.status==='next'?' upcoming':'');b.textContent=p.name;b.setAttribute('aria-label',p.name+', '+p.group+(p.status==='next'?', next trip':', visited'));b.addEventListener('click',()=>p.kind==='Country'?showCountry(p.group):markers[i].openPopup());overlay.append(b);return b});
  ...
  function layoutLabels(){
   ...
   const countryMode=currentView==='all'&&map.getZoom()<=overviewZoom+0.75;
   const shown=countryMode?countries.map((_,i)=>places.length+i):active;
   markers.forEach((m,i)=>{if(shown.includes(i)){if(!map.hasLayer(m))m.addTo(map)}else if(map.hasLayer(m))map.removeLayer(m)});
   container.dataset.labelMode=countryMode?'countries':'places';
   const points=shown.map(i=>({i,p:map.latLngToContainerPoint([entries[i].lat,entries[i].lng])})).filter(({p})=>p.x>8&&p.x<size.x-8&&p.y>8&&p.y<size.y-8);
   ...
   for(const {i,p} of points){
    ...
    const line=document.createElementNS('http://www.w3.org/2000/svg','line');line.setAttribute('x1',p.x);line.setAttribute('y1',p.y);line.setAttribute('x2',x);line.setAttribute('y2',y);line.setAttribute('stroke',entries[i].status==='next'?'#ae9142':'#546b86');lines.append(line);
   }
  }
  function showCountry(group){
   currentView='country';active=places.map((p,i)=>p.group===group?i:-1).filter(i=>i>=0);
   map.closePopup();map.fitBounds(active.map(i=>[places[i].lat,places[i].lng]),{padding:[65,65],maxZoom:6,animate:false});
   document.querySelectorAll('[data-view]').forEach(b=>b.setAttribute('aria-pressed','false'));layoutLabels();
  }
  function setView(view){
   ...
   if(view==='all')overviewZoom=map.getZoom();
